...
# Check if host site is available to retrieve HEFS files from sftp.
...
#for HEFS upload dates are more erratic so lookback is just the number of locations and that is multiplied by the number of instances you want to retrieve
    hrsback = int(lookback)*31
...
#        Store file in FEWS import/hefs/gzipped Directory
        with open(tempDir +file, 'wb') as f: #importDir
            f.write(grib)
...
#unzip files in import/hefs

    for file in os.listdir(tempDir): #importDir
        with gzip.open(tempDir+file, 'rb') as g:  #importDir
            file_content = g.read()
            with open(importDir+file+'.xml', 'wb') as z:
